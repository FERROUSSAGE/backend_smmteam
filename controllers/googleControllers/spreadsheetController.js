...
const getPatternHeader = (date) => {
    const dayMonth = date.toLocaleString('ru-RU', { day: 'numeric', month: 'long', timeZone: 'Europe/Moscow' }).toUpperCase();
    return {
...
class SpreadsheetController {
    async accessSpreadsheet(req, res, next) {
        ...
        try {
            ...
            const newSheetIndex = (await copySheet(doc.sheetCount - 1, date.toLocaleDateString('ru-RU', { timeZone: 'Europe/Moscow' })));
            const newSheet = await doc.sheetsByIndex[newSheetIndex];
            await newSheet.updateProperties({ title: date.toLocaleDateString('ru-RU', { timeZone: 'Europe/Moscow' }) });
